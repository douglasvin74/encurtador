...
public class Contador {
    ...
    public void consumir(String bootstrap) {
        consumir(bootstrap, GRUPO);
    }

    /**
     * Mesmo consumo, com o grupo escolhido por quem chama.
     *
     * Existe por causa do auto-teste: um Contador de verdade rodando na maquina
     * esta no grupo padrao, e membros do mesmo grupo DIVIDEM as
     * particoes - o teste receberia so uma parte dos proprios cliques e falharia
     * sem nada estar quebrado. Com grupo proprio, ele le o topico inteiro.
     */
    public void consumir(String bootstrap, String grupo) {
        try (KafkaConsumer<String, String> consumidor = new KafkaConsumer<>(config(bootstrap, grupo))) {
    ...
    private static Properties config(String bootstrap, String grupo) {
    ...
    private void rotear(HttpExchange troca) throws IOException {
        ...
        if (!troca.getRequestMethod().equals("GET")) {
        ...
        } else if (caminho.equals("/health")) {
            // O lado da leitura nao depende de banco: se o HTTP responde, ele esta de pe.
            responder(troca, 200, "ok\n");
            ...
            responder(troca, 404, "GET /stats, GET /stats/<codigo> ou GET /health\n");
        }
    }
